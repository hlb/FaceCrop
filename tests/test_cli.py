...
class TestCLI(unittest.TestCase):
    def setUp(self):
        ...
        self.fixture_image = os.path.join(
            os.path.dirname(__file__), "fixtures", "images", "Mona_Lisa.jpg"
        )
        if not os.path.exists(self.fixture_image):
            raise FileNotFoundError(
                f"Test fixture image not found: {self.fixture_image}"
            )
    ...
    def test_directory_with_mixed_files(self, mock_argv):
        ...
        self.assertEqual(
            cm.exception.code, 0
        )  # Should succeed because at least one image processed
