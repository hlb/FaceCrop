...
class TestImageProcessor(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.mona_lisa_image = os.path.join('tests', 'fixtures', 'images', 'Mona_Lisa.jpg')
        if not os.path.exists(cls.mona_lisa_image):
            raise FileNotFoundError(f"Test fixture image not found: {cls.mona_lisa_image}")
    ...
    def test_process_mona_lisa_strict(self):
        ...
        result = self.processor.process_image(self.mona_lisa_path, self.output_path, strict=True)
        self.assertTrue(result)
        self.assertTrue(os.path.exists(self.output_path))
    ...
    def test_process_cat_strict(self):
        ...
        result = self.processor.process_image(self.cat_path, self.output_path, strict=True)
    ...
    def test_process_image_with_circular_mask(self):
        ...
        result = self.processor.process_image(self.mona_lisa_path, self.output_path, circular_mask=True)
        self.assertTrue(result)
        self.assertTrue(os.path.exists(self.output_path))
